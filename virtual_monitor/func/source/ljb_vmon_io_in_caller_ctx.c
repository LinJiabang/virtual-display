/*!
    \file       lci_vmon_io_in_caller_ctx.c
    \brief      EvtIoInCallerContext implementation.
    \details    EvtIoInCallerContext implementation.
    \authors    lucaslin
    \version    0.01a
    \date       May 29, 2013
    \todo       (Optional)
    ...
    + 0.01a;    July 25, 2013;   lucaslin
     - Created.
    + 0.01a;    August 9, 2013;   robert
     - Fix a BSOD caused by LJB_VMON_DrawCursor().
...
LJB_VMON_IoInCallerContext(
    ...
    switch (params.Parameters.DeviceIoControl.IoControlCode)
        {
    case IOCTL_LCI_USBAV_ACQUIRE_FRAME:
        if (pDevCtx->DeviceDead)
            {
            LJB_VMON_Printf(pDevCtx, DBGLVL_ERROR,
                ("?" __FUNCTION__
                ": DeviceGone? fail IOCTL_LCI_USBAV_ACQUIRE_FRAME directly.\n"
                ));
    ...
    case IOCTL_LCI_USBAV_RELEASE_FRAME:
...
/*
 ...
 Definition:
    ...
    LJB_VMON_DrawCursor(
        __in WDFDEVICE            			WdfDevice,
        __in LCI_USBAV_PRIMARY_SURFACE *	pPrimarySurface
        );
...
/*
 ...
 Description:
    IOCTL_LCI_USBAV_ACQUIRE_FRAME request, we find a free LCI_USBAV_PRIMARY_SURFACE
...
LJB_VMON_IoctlAcquireFramePreProcess(
	...
	LCI_USBAV_PRIMARY_SURFACE *     pMaxFrameIdSurface;
    ...
    if (pPrimarySurface == NULL)
		...
		*/
		LJB_VMON_Printf(pDevCtx, DBGLVL_FLOW,
			(" " __FUNCTION__ ": MediaPlayerState(%d).\n",
			pDevCtx->MediaPlayerState
			));
		...
		*/
		if (pDevCtx->MediaPlayerState == 0)
			...
			*/	
			...
			{
			LJB_VMON_Printf(pDevCtx, DBGLVL_ERROR,
				("?" __FUNCTION__ ": No available pPrimarySurface? put in queue.\n"));
				
			LJB_VMON_Printf(pDevCtx, DBGLVL_ERROR,
				("?" __FUNCTION__ ": No available pPrimarySurface?\n"));
			...
			*/
			...
			if (pWaitRequest == NULL)
				...
				WdfRequestCompleteWithInformation(WdfRequest, ntStatus, 0);
				return TRUE;
				}
	
			pWaitRequest->IoctlCode = IOCTL_LCI_USBAV_ACQUIRE_FRAME;
			pWaitRequest->Request = WdfRequest;
			pWaitRequest->pInputWaitUpdateData = NULL;
			pWaitRequest->pOutputWaitUpdateData = pOutputBuffer;
			...
			*/
			...
			LJB_VMON_Printf(pDevCtx, DBGLVL_FLOW,
				(" " __FUNCTION__
				": queue pWaitRequest(%p)->Request(%p) for later completion\n",
				pWaitRequest,
...
#if 0		
	/*
	 Temporary patch for customer to display black image after the monitor is
	 disabled in "Screen Resolution" Window.
	*/	
	if (!pDevCtx->VisibilityVisible)
		{		
		LJB_VMON_Printf(pDevCtx, DBGLVL_FLOW,
		    ("?" __FUNCTION__ ": FrameId(%d) acquired. Black image.\n",
			pPrimarySurface->FrameId
			));
			
		LJB_VMON_Printf(pDevCtx, DBGLVL_FLOW,
            (" " __FUNCTION__ ": Visibility is off. Send black image for now.\n"
			));
...
LJB_VMON_IoctlReleaseFramePreProcess(
    ...
    pPrimarySurface = (LCI_USBAV_PRIMARY_SURFACE *)
