...
/*!
 Name:  IOCTL_LCI_USBAV_FRAME_OUTPUT

 \details
    User space app sends LCI_FRAME_OUTPUT buffer which includes USB AV command
    message in lpInputBuffer. The USB AV command message is placed at the offset
    sizeof(ULONG) after lpInBuffer, and the size of Command message is specified
    by nInBufferSize.
    The Response message is specified by lpOutBuffer, and the response buffer
    size is specified by nOutBufferSize. For best efficiency, we use
    METHOD_NEITHER.
    ...
    STATUS_INVALID_PARAMETER.
 */
#define IOCTL_LCI_USBAV_FRAME_OUTPUT                \
    CTL_CODE(FILE_DEVICE_UNKNOWN,                   \
    LCI_USBAV_IOCTL_BASE + 5,                       \
    METHOD_NEITHER,                                 \
    FILE_ANY_ACCESS)

#pragma warning (disable: 4200)
typedef struct _LCI_FRAME_OUTPUT
    {
    ULONGLONG           FrameTimeStamp;
    UCHAR               UsbAvCommandMsg[0];
    } LCI_FRAME_OUTPUT;

#endif /* _LCI_USBAV_IOCTL_H_ */
